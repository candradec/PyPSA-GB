...
def write_buses(year, networkmodel="Reduced"):
    ...
    if networkmodel == "Reduced":
        ...
        df.to_csv("data/UC_data/buses.csv", index=False, header=True)
        ...
        if year <= 2020:
            df["carrier"] = "AC"
        df.to_csv("data/LOPF_data/buses.csv", index=False, header=True)

        df_buses = pd.read_csv("data/network/BusesBasedGBsystem/network/buses.csv")
        df_buses.to_csv("data/LOPF_data/buses.csv", index=False, header=True)
