...
def read_tidal_lagoon(year, scenario, fes):
    ...
    if fes == 2021:
        ...
        df_tidal_lagoon_capacities.index = pd.to_datetime(
            df_tidal_lagoon_capacities.index, infer_datetime_format=True, utc=True
        )
...
def read_tidal_stream(year, scenario, fes):
    ...
    if fes == 2021:
        ...
        df_tidal_stream_capacities.index = pd.to_datetime(
            df_tidal_stream_capacities.index, infer_datetime_format=True, utc=True
        )
...
def add_marine_timeseries(year, year_baseline, scenario, time_step):
    ...
    df_tidal_lagoon.index = pd.to_datetime(
        df_tidal_lagoon.index, infer_datetime_format=True
    )
    ...
    df_tidal_stream.index = pd.to_datetime(
        df_tidal_stream.index, infer_datetime_format=True
    )
    ...
    df_wave_power.index = pd.to_datetime(
        df_wave_power.index, infer_datetime_format=True
    )
    df_LOPF.index = pd.to_datetime(df_LOPF.index, infer_datetime_format=True)
